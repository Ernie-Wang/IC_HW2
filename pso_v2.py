...
class PSO():
    def __init__(self, dim, num, max_iter, u_bound, l_bound, func, end_thres, end_sample, fit_max):
        ...
        self.remain = np.zeros(self.num)              # Particle remain unchanged time
        self.remain_threshold = 3                    # Particle remain unchanged threshold
    ...
    def pso_init(self, X=None):
        ...
        if X is None:
            for n in range(self.num):
                self.X[n] = np.random.uniform(low=self.l_bound, high=self.u_bound, size=(self.dim))
                self.V[n] = np.random.uniform(low=-1, high=1, size=(self.dim))
            ...
            for part in range(self.num):
                ...
                if dist == 0:
                    ...
                    if self.remain[part] > self.remain_threshold:
                        ...
                        self.V[part] = np.random.uniform(low=-1, high=1, size=(self.dim))
                        self.pbest[part] = tmp_X[part].copy()
                        self.pbest_v[part] = self.func(self.pbest[part])
                        self.remain[part] = 0
            self.X = tmp_X.copy()
